(function () {
    ...
    var resourcesRoot = '../ReactDesktopApps.Resources/';
    var winFormsAssemblyInfoPath = '../ReactDesktopApps.AppWinForms/Properties/AssemblyInfo.cs';
    ...
    gulp.task('www-copy-resources-lib', () => {
        return gulp.src('../ReactDesktopApps.Resources/bin/Release/ReactDesktopApps.Resources.dll')
    ...
    gulp.task('www-copy-resources', () => {
        return gulp.src(['./wwwroot/**/*',
            ...
            '!./wwwroot/*.asax',
            '!./wwwroot/platform.*'])
            .pipe(gulp.dest('../ReactDesktopApps.Resources/'));
    });
    ...
    gulp.task('www-msbuild-web', () => {
        return gulp.src('./ReactDesktopApps.csproj')
            .pipe(msbuild({
    ...
    gulp.task('www-msbuild-resources', () => {
        return gulp.src('../ReactDesktopApps.Resources/ReactDesktopApps.Resources.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-msbuild-console', () => {
        return gulp.src('../ReactDesktopApps.AppConsole/ReactDesktopApps.AppConsole.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-msbuild-winforms', () => {
        return gulp.src('../ReactDesktopApps.AppWinforms/ReactDesktopApps.AppWinforms.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-nuget-restore', () => {
        return gulp.src('../../ReactDesktopApps.sln')
            .pipe(nugetRestore());
    });
    ...
    gulp.task('www-nuget-pack-winforms', function (callback) {
        ...
        var excludes = globule.find([
            '../ReactDesktopApps.AppWinForms/bin/x86/Release/*.pdb',
            '../ReactDesktopApps.AppWinForms/bin/x86/Release/*.vshost.*'
        ]);
        var includes = [
        { src: '../ReactDesktopApps.AppWinForms/bin/x86/Release/*.*', dest: '/lib/net45/' },
        { src: '../ReactDesktopApps.AppWinForms/bin/x86/Release/locales/*.*', dest: '/lib/net45/locales/' }];
        return nugetpack({
            id: title,
            title: title,
                version: version,
